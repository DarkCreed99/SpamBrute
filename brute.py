...
try:
    ...
    print(f"{Y}[{R}!{Y}] Bahan Belum Terinstall{merah}...")
    time.sleep(5)
    print(f"{Y}[{R}!{Y}] Type{R}:{Y}pip{W} install colorama requests")
...
def mr_wibu():
    ...
    if mr_tytyd=="n" or mr_tytyd=="N":
        sys.exit(f"{Y}[{R}!{Y}] Keluar dari script{biru}....{Y}")
        time.sleep(5)
...
""")
        ...
        sys.exit(f"{Y}[{R}!{Y}] Eror Cek Jaringan{abu}....{abu}")
    except KeyboardInterrupt:
        sys.exit(f"{Y}[{R}!{Y}] Kluar{abu}...{W}")
    ...
    try:
        ...
        for i in range(int(power_python)):
            ...
            if "call" in jag:
                ...
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam Call Ke No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "Berhasil Ngab" in gas:
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam WA Ke No {Y}{mr_am}")
            else:
                mengetik(f"{B}[{G}✓{B}] Mengirim Spam WA Ke No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "ok" in dekor2:
                mengetik(f"{B}[{G}✓{B}] Mengirim Spam Sms Ke No {Y}{mr_am}")
            else:
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam Sms Ke No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "neng" in buka:
                ...
                mengetik(f"{B}[{G}✓{B}] Mengirim Spam WAKe No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "erors" in gaskeun:
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam SmsKe No {Y}{mr_am}")
            else:
                mengetik(f"{B}[{G}✓{B}] Mengirim Spam Sms Ke No {Y}{mr_am}")
            time.sleep(1)
            ...
            if "PENDING" in AmmarGamteng:
                ...
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam Sms Ke No {Y}{mr_am}{W}")
            time.sleep(1)
            ...
            if "cus" in Gwganz:
                mengetik(f"{Y}[{R}×{Y}] Gagal Mengirim Spam SnsKe No {Y}{mr_am}")
            else:
                mengetik(f"{B}[{G}✓{B}] Mengirim Spam Sms Ke No {Y}{mr_am}")
            time.sleep(2)
        ...
        sys.exit(f"{Y}[{R}!{Y}] Eror Cek Jaringan lu Ngab{abu}....{abu}")
    except KeyboardInterrupt:
        sys.exit(f"{Y}[{R}!{Y}] Keluar!!{abu}...{W}")
    except ValueError:
